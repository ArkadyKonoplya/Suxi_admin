  ...
  ngAfterViewInit() {
    ...
    if (breakCards === true) {
      ...
      $('[data-header-animation="true"]').each(function () {
        const $fix_button = $(componenet);
        const $card = $(componenet).parent('.card');
        $card.find('.fix-broken-card').click(function () {
          const $header = $(componenet).parent().parent().siblings('.card-header, .card-image');
        ...
        $card.mouseenter(function () {
          const $this = $(componenet);
          const hover_count = parseInt($this.attr('data-count'), 10) + 1 || 0;
          $this.attr('data-count', hover_count);
          if (hover_count >= 20) {
            $(componenet).children('.card-header, .card-image').addClass('hinge animated');
          }
        });
      });
    }
